...
class ApiKeyManager:
    ...
    """
    def __init__(self, sheet_url="https://docs.google.com/spreadsheets/d/1gqlLToS3OXPA-CvfgXRnZ1A6n32eXMTkXz4ghqZxe2I/gviz/tq?tqx=out:csv&gid=0", revival_delay_seconds=60, max_failures=3):
        self.revival_delay_seconds = revival_delay_seconds
        self.max_failures = max_failures

        self._keys_info = self._load_api_keys_from_sheet(sheet_url)
        if not self._keys_info:
            raise ValueError("No API keys were loaded. Please check the Google Sheet URL and format.")

        self._key_queue = queue.Queue()
        for key_info in self._keys_info:
            self._key_queue.put(key_info)

        self._retired_keys_lock = threading.Lock()
        self._retired_keys = []

        self._failure_counts = {}  # Track how many times each key has failed

        self._stop_event = threading.Event()
        self._revival_thread = threading.Thread(target=self._revive_keys_periodically, daemon=True)
        self._revival_thread.start()

        console.print(f"[ApiKeyManager] Initialized with [bold green]{self._key_queue.qsize()}[/bold green] active keys. Key revival delay: {revival_delay_seconds}s.")

    def _load_api_keys_from_sheet(self, sheet_url):
        console.print("[ApiKeyManager] Downloading API keys from Google Sheets...")
        try:
    ...
    def _revive_keys_periodically(self):
        while not self._stop_event.wait(5):  # Check every 5 seconds
            with self._retired_keys_lock:
                ...
                keys_to_revive = []
                remaining_retired_keys = []

                for retirement_time, key_info in self._retired_keys:
                    key_name = key_info["name"]
                    if current_time - retirement_time >= self.revival_delay_seconds:
                        # Only revive keys that have not exceeded max failures
                        if self._failure_counts.get(key_name, 0) < self.max_failures:
                            keys_to_revive.append(key_info)
                    else:
                        remaining_retired_keys.append((retirement_time, key_info))

                self._retired_keys = remaining_retired_keys

            for key_info in keys_to_revive:
                self._key_queue.put(key_info)
                console.print(f"[ApiKeyManager] Reviving key [bold cyan]'{key_info['name']}'[/bold cyan]. Active keys: {self._key_queue.qsize()}")
    ...
    def get_key(self):
        ...
        try:
            ...
            if fail_count >= self.max_failures:
                console.print(f"[ApiKeyManager] 🔥 Permanently removing key [bold red]'{key_name}'[/bold red] after {fail_count} failures.", style="red")
                # Do NOT retire or reuse the key
            else:
                with self._retired_keys_lock:
                ...
                console.print(
                    f"[ApiKeyManager] Retiring key [bold red]'{key_name}'[/bold red] for {self.revival_delay_seconds}s (fail count: {fail_count}). Active: {active}, Cooldown: {cooldown}"
                )
            raise
    ...
    def shutdown(self):
        console.print("[ApiKeyManager] Shutting down key revival thread...")
